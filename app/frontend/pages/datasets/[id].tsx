    ...
    <div style={{ border: "1px solid #eee", borderRadius: 8, padding: 12 }}>
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
        ...
        {isAuthed ? (
          ...
          <span style={{ color: "#666" }}>Log in to run jobs</span>
        )}
      </div>
      ...
      {jobId && (
        <div>
          ...
          {summary && (
            <p>
              <b>stats:</b> count={summary.count} min={summary.min} max={summary.max} mean={summary.mean}
            </p>
          )}
  ...
  useEffect(() => {
    ...
    fetch(`${API}/datasets/${id}/preview`)
      .then(async (r) => {
        ...
        if (!r.ok || (json as any)?.detail) {
          setPreviewError((json as any)?.detail || `Preview failed (${r.status})`);
        } else {
          setPreview(json);
    ...
    fetch(`${API}/datasets/${id}/files`)
      .then(async (r) => {
        const json = await r.json();
        if (!r.ok || (json as any)?.detail) setFilesError((json as any)?.detail || `Files failed (${r.status})`);
        else {
    ...
    fetch(`${API}/datasets/${id}/geojson`)
      .then(async (r) => {
        const json = await r.json();
        if (!r.ok || (json as any)?.detail) setGeoError((json as any)?.detail || `GeoJSON failed (${r.status})`);
        else {
          setGeo(json);
          setGeoError(null);
        }
    ...
    <main style={{ padding: "16px", fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <section style={{ marginBottom: 24 }}>
        ...
        {files.length > 0 && (
          <ul>
            {files.map((f) => (
              <li key={f.file_id}>
                {f.original_name} <small>({f.bytes} bytes)</small>
              </li>
            ))}
          </ul>
        )}
      ...
      <section style={{ marginBottom: 24 }}>
        ...
        {Array.isArray((preview as any)?.columns) && Array.isArray((preview as any)?.data) ? (
          ...
          !previewError &&
          preview !== null && <p style={{ color: "#b00" }}>Preview not available for this file.</p>
        )}
      </section>
